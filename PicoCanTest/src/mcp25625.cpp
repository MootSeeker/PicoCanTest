...
MCP2515::MCP2515( ) : CANControllerClass( ), _spiSettings( 10E6, MSBFIRST, SPI_MODE0 ), _csPin( MCP2515_DEFAULT_CS_PIN ),
                                             _intPin( MCP2515_DEFAULT_INT_PIN ), _clockFrequency( MCP2515_DEFAULT_CLOCK_FREQUENCY )
{
}
...
int MCP2515::begin( long baudRate )
{
  ...
  if( readRegister( REG_CANCTRL ) != 0x80 ) return CAN_ERROR_FAILINIT;
  ...
  if( cnf == NULL ) return CAN_ERROR_FAILINIT;
  ...
  if( readRegister( REG_CANCTRL ) != 0x00 ) return CAN_ERROR_FAILINIT;
